...
extractMap :: OperadElement a n t -> MonomialMap a t n
extractMap (OE m) = m
...
(.*.) :: (Num n) => n -> OperadElement a n t -> OperadElement a n t
...
foldMonomials :: ((OrderedTree a t,n) -> [b] -> [b]) -> OperadElement a n t -> [b]
...
fromList :: (TreeOrdering t, Ord a, Eq n, Num n) => [(OrderedTree a t,n)] -> OperadElement a n t
fromList = OE . Map.filter (/=0) . Map.fromListWith (+)

-- | Given an operad element, extract a list of (tree, coefficient) pairs.
toList :: OperadElement a n t -> [(OrderedTree a t, n)]
...
oe :: (Ord a, TreeOrdering t, Eq n, Num n) => [(OrderedTree a t, n)] -> OperadElement a n t
oe = fromList
...
zero :: (Ord a, TreeOrdering t, Eq n, Num n) => OperadElement a n t
zero = oe [(ot $ leaf 1, 0)]

-- | Check whether an element is equal to 0.
isZero :: (Eq n, Num n) => OperadElement a n t -> Bool
isZero (OE m) = Map.null $ Map.filter (/=0) m

-- | Extract the leading term of an operad element as an operad element.
leadingOTerm :: (Ord a, TreeOrdering t, Eq n, Num n) => OperadElement a n t -> OperadElement a n t
leadingOTerm om = oe [leadingTerm om]

-- | Extract the leading term of an operad element.
leadingTerm :: (TreeOrdering t, Num n) => OperadElement a n t -> (OrderedTree a t, n)
